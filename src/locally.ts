...
import { getStorableTypeAndValue } from './utils/getStorableTypeAndValue';
import { persistCurrentConfig, removeValue } from './utils/storage';

// Fastest utility functions for this case.
// Faster than underscore and lodash.
// Besides, do I even need them here?
const utils = {
  each: <T>(arr: T[], iteratee: (item: T, index?: number) => void) => {
    var l = arr.length;
    while (l--) iteratee(arr[l], l);
  },
  map: function <T>(arr: T[], iteratee: (item: T, index?: number) => any) {
...
Locally.prototype.set = function (key, value, options) {
  ...
  var res = getStorableTypeAndValue(value);
...
Locally.prototype.persist = function (key) {
  return _config[key]
    ? delete _config[key].ttl && persistCurrentConfig() && _clearTimeout(key)
    : false;
};

Locally.prototype.expire = function (key, ttl) {
  return _config[key]
    ? !!(_config[key].ttl = Date.now() + ttl) && persistCurrentConfig()
    : false;
};
