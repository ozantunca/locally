...
(function () {
  ...
  const ms = require('ms') as (input: string | number) => string | number;
  const lzstring = require('lz-string') as {
    compressToUTF16: (str: string) => string;
    decompressFromUTF16: (str: string) => string | null;
  };
  ...
  const _timeouts: Record<string, NodeJS.Timeout> = {};
  ...
  Locally.prototype.ttl = function (key: string, returnString?: boolean): number | string {
    ...
    return !returnString ? remaining : (ms(remaining) as string);
  };
  ...
  if (typeof exports === 'object') {
    (exports as { Store: typeof Locally }).Store = Locally;
  }
  ...
  if (typeof window === 'object') {
    (window as unknown as { Locally: { Store: typeof Locally } }).Locally = {
      Store: Locally
    };
  }
})();
