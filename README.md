...
[![CI](https://github.com/ozantunca/locally/actions/workflows/ci.yml/badge.svg)](https://github.com/ozantunca/locally/actions/workflows/ci.yml)
...
Locally is installable via npm:

- `npm install locallyjs`
...
In the browser with a script tag (IIFE build), the global is `window.Locally`:

```js
const Store = window.Locally.Store;
const store = new Store();
```
...
Since version v0.3.3 `locally` has a `light` version that does not support compression and `ms` module for those who don't want these features and want to save size:

```js
import { Store } from "locallyjs/light";
const store = new Store();
```

Or with CommonJS: `const { Store } = require("locallyjs/light");`
...
After cloning the repo and running `npm install`, run:
